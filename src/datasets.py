...
class MidiDataModule(pl.LightningDataModule):
  ...
  def setup(self, stage=None):
    valid_files = [f for f in self.files if os.path.basename(f).startswith('e')]
    test_files = [f for f in self.files if os.path.basename(f).startswith('f')]
    train_files = [f for f in self.files if f not in valid_files + test_files]
