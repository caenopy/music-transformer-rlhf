...
class Seq2SeqModule(pl.LightningModule):
  def __init__(self,
    ...
    # Initialize model with music transformer parameters:
    ...
    # Define the number of bars processed at a time
    self.max_bars = self.context_size
    # Define the number of positions in input sequence the model can handle
    self.max_positions = 512
    # Embedding to map each bar in the input sequence to a vector of size d_model
    self.bar_embedding = nn.Embedding(self.max_bars + 1, self.d_model)
    # Embedding to map each position in the input sequence to a vector of size d_model
